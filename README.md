...
- CCIP entrypoints emit named events for indexer-friendly auditing: `CCIPMinted(account, amount, ccipEstimatedUsedHeadroom)` from inbound mints and `CCIPBurned(account, amount, ccipEstimatedUsedHeadroom)` from all three burn overloads.
