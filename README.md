...
- The ON locked on BSC *via CCIP* equals the wON minted on Ethereum *via CCIP*, message-for-message — but that equality is guaranteed by CCIP's 1:1 message pairing, **not** by any on-chain check on Ethereum. **Ethereum cannot read the BSC locked balance**, so the bridge trusts Chainlink (the DON + RMN) to honour the pairing. The on-chain `MAX_CCIP_MINTED` / `ccipMintedSupply` counter is a *local approximation* of that off-chain figure — it caps the local CCIP-mint counter, not aggregate wON supply.
