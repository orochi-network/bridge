...
#   Step 2  re-wire the existing BSC LockReleaseTokenPool's ETH lane (script 05 ONLY — never
#           `make deploy-bsc`, never script 04 on BSC: path-4 blocker, CannotResolveCCIPAdmin).
...
Finish RUNBOOK §4.4:
  Step 2  BSC re-wire (script 05 ONLY — do NOT use 'make deploy-bsc', do NOT run script 04):
            forge script script/05_ApplyChainUpdates.s.sol --rpc-url bsc --broadcast --account $ACCOUNT
