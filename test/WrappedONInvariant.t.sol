...
///         `ccipEstimatedUsedHeadroom` is tracking.
///
///         The invariant under test is the bridge safety invariant:
///
///             lockedON_BSC + reserveON_ETH >= totalSupply(wON)
///
///         `ccipEstimatedUsedHeadroom` is a BSC-pool-balance approximation rather than a
...
contract WrappedONHandler is Test {
    ...
    function ccipMint(uint256 actorSeed, uint256 amount) external {
        ...
        uint256 headroom = cap - WON.ccipEstimatedUsedHeadroom();
        if (headroom == 0) {
            return;
        }
    ...
    ///
    ///      Importantly: `bscLocked` is NOT decremented here, so `ccipEstimatedUsedHeadroom`
    ///      drifts BELOW `bscLocked` and the saturating-decrement branch in
    ///      `_decrementUsedHeadroom` becomes fuzzer-reachable. The `invariant_BackingCoversSupply`
    ...
    function adversarialPoolBurn(uint256 actorSeed, uint256 amount) external {
        ...
        vm.prank(user);
        WON.transfer(POOL, amount);
        // Note: `bscLocked` deliberately NOT decremented — pool's `_decrementUsedHeadroom`
        // saturates at 0 when `amount > ccipEstimatedUsedHeadroom`, which is the property under
        // test here.
        vm.prank(POOL);
        WON.burn(amount);
    }
    ...
    ///      which independently calls `_decrementUsedHeadroom`. Without coverage here the
    ...
    ///      allowance check) so its `_decrementUsedHeadroom` path is fuzzer-reachable.
...
contract WrappedONInvariantTest is StdInvariant, Test {
    ...
    function setUp() public {
        ...
        // `_decrementUsedHeadroom` branch is fuzzer-reachable) + two-step CCIP admin
    ...
    /// @notice `ccipEstimatedUsedHeadroom` is BOUNDED ABOVE by `bscLocked`. The previous
    ...
    ///         `_decrementUsedHeadroom` can push `ccipEstimatedUsedHeadroom` strictly below
    ///         `bscLocked`; the bound `ccipEstimatedUsedHeadroom <= bscLocked` still holds because
    ...
    function invariant_CounterBoundedByBscLocked() public view {
        assertLe(
            won.ccipEstimatedUsedHeadroom(),
            handler.bscLocked(),
            "invariant: ccipEstimatedUsedHeadroom exceeds simulated BSC pool balance"
        );
    }
    ...
    function invariant_CcipMintedSupplyWithinCap() public view {
        assertLe(
            won.ccipEstimatedUsedHeadroom(),
            won.MAX_CCIP_MINTED(),
            "invariant: ccipEstimatedUsedHeadroom exceeds MAX_CCIP_MINTED"
        );
    }
