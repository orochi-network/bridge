...
**Note on `ccipMintHeadroomUsed` monitoring** (SECURITY: WON-3 / CCIP-7). The contract-side
`ccipMintHeadroomUsed` counter approximates BSC locked balance but saturating-decrements on
...
authoritative locked-balance read; treat `ccipMintHeadroomUsed` as a useful local indicator
