...
- **M4** `_credit` recipient hardening — both contracts reject `address(0)` and `address(this)` with `BadRecipient`. No silent redirect to `0xdead`: a redirect at `_credit` would unlock real ON / mint wON to `0xdead` while `OFTCore` dispatched `sendCompose` to the ORIGINAL bad address (it captures `toAddress` pre-credit), leaving the compose call stuck and conservation accounting visibly damaged. Reverting keeps the LZ message retryable-pending; operators must detect the stuck message off-chain.
