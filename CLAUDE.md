...
- **M4** `_credit` recipient hardening — both contracts redirect `address(0)` and `address(this)` to `address(0xdead)` for plain messages and revert with `BadRecipientWithCompose` for composed messages. On `WrappedON` the plain-message redirect forces the mint branch so reserve is never sent to a dead recipient; the composed-message revert keeps the LZ message in retryable-pending state so no orphan wON is minted at `0xdead` while the compose dispatch ends up stuck against the unredirected bad address.
