...
- **CCIP infrastructure addresses in `script/Helper.sol` are populated** from https://docs.chain.link/ccip/directory for the four supported chains (ETH mainnet, BSC mainnet, Sepolia, BSC testnet). **Re-verify against the live chain before broadcasting** with `make validate-config RPC=<target>`. Only the testnet `onToken` fields are `address(0)` (no canonical ON on Sepolia/BSC-testnet — deploy a mock). Scripts call `_requireSet` on every address they consume, so any unset field still fails fast.
