...
- **Location:** `src/WrappedON.sol` (`deposit`); `script/06_TransferOwnership.s.sol`.
...
- **Recommendation (original):** (Was implemented) restrict `deposit` to a protocol-managed `LIQUIDITY_MANAGER_ROLE`. Per-window ETH→BSC redemption is then bounded by what the role-holder wraps plus the BSC inbound rate limits (see RUNBOOK §4.4).
- **Residual risk note A — permissionless deposit:** With `deposit` permissionless and uncapped, wON supply growth and ETH→BSC redemption demand are bounded only by the ETH-side ON circulating supply (600M) and the configured CCIP pool rate limits. The aggregate safety invariant (`lockedON_BSC + reserveON_ETH >= totalSupply(wON)`) continues to hold mechanically, but burst ETH→BSC redemption pressure is no longer capped by role access. Operators must size BSC pool liquidity and ETH→BSC rate limits conservatively (see RUNBOOK §4.4).
